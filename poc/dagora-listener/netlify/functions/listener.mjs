...
export default async (req) => {
    ...
    const isAuthRefresh = url.pathname.includes("/auth/refresh");
    ...
    // Return plausible auth response for the refresh endpoint
    let responseBody;
    if (isAuthRefresh) {
        responseBody = {
            success: true,
            data: {
                access_token: "captured_" + Date.now(),
                token_type: "Bearer",
                expires_in: 3600,
            },
        };
    ...
    const resp = Response.json(responseBody, { status: 200, headers: CORS });
